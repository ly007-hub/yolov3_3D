...
def darknet53(pretrained=False, hr=False, **kwargs):
    ...
    if pretrained:
        ...
        if hr:
            ...
            model.load_state_dict(
                torch.load(path_to_dir + '/weights/darknet53/darknet53_75.42.pth', map_location='cuda'), strict=False)
    return model
...
def darknet_tiny(pretrained=False, hr=False, **kwargs):
    ...
    if pretrained:
        ...
        if hr:
            ...
            model.load_state_dict(torch.load(path_to_dir + '/weights/darknet_tiny_hr_61.85.pth', map_location='cuda'),
                                  strict=False)
        else:
            print('Loading the darknet_tiny ...')
            model.load_state_dict(
                torch.load(path_to_dir + '/weights/darknet_tiny_63.50_85.06.pth', map_location='cuda'), strict=False)
    return model
