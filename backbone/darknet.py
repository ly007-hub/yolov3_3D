...
class Conv_BN_LeakyReLU(nn.Module):
    def __init__(self, in_channels, out_channels, ksize, padding=0, stride=1, dilation=1):
        ...
        self.convs = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, ksize, padding=padding, stride=stride, dilation=dilation),
...
if __name__ == '__main__':
    # print("test start")
    # input = torch.ones([3, 3, 64, 64])
    # model = darknet53()
    # output = model(input)

    input_3D = torch.ones([1, 3, 64, 64, 64])
    yolov_3Dnet = darknet53()
    output_3D = yolov_3Dnet(input_3D)
