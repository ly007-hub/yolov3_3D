...
class myEvaluator():
    def __init__(self, dataset, data_root, img_size, device, transform, labelmap, set_type='test', year='2021', display=False):
        ...
        # path
        self.devkit_path = data_root + '/pnens/' + year
        ...
        self.output_dir = self.get_output_dir('pnens_eval/', self.set_type)
    ...
    def evaluate(self, net):
        ...
        for i in range(num_images):
            im, gt, h, w, d = self.dataset.pull_item(i)
        ...
        net = model
        net.eval()
        num_images = len(evaluator.dataset)
        evaluator.all_boxes = [[[] for _ in range(num_images)]
                          for _ in range(1)]

        # timers
        det_file = os.path.join(evaluator.output_dir, 'detections.pkl')

        for i in range(num_images):
            # if i == 0: break
            im, gt, h, w, d = evaluator.dataset.pull_item(i)

            x = Variable(im.unsqueeze(0)).to(evaluator.device)
            t0 = time.time()
            # forward
            net.trainable = False
            bboxes, scores, cls_inds = net(x)
            ...
            for j in range(1):
                ...
                inds = np.where(cls_inds == j)[0]
                if len(inds) == 0:
                    evaluator.all_boxes[j][i] = np.empty([0, 5], dtype=np.float32)
                    continue
                c_bboxes = bboxes[inds]
                c_scores = scores[inds]
                c_dets = np.hstack((c_bboxes,
                                    c_scores[:, np.newaxis])).astype(np.float32,
                                                                     copy=False)
                evaluator.all_boxes[j][i] = c_dets

            if i % 500 == 0:
                print('im_detect: {:d}/{:d} {:.3f}s'.format(i + 1, num_images, detect_time))

        with open(det_file, 'wb') as f:
            pickle.dump(evaluator.all_boxes, f, pickle.HIGHEST_PROTOCOL)

        print('Evaluating detections')
        evaluator.evaluate_detections(evaluator.all_boxes)

        print('Mean AP: ', evaluator.map)
        """
    ...
    def do_python_eval(self, use_07=True):
        ...
        """
        cachedir = os.path.join(evaluator.devkit_path, 'annotations_cache')
        aps = []
        # The PASCAL VOC metric changed in 2010
        use_07_metric = use_07
        print('VOC07 metric? ' + ('Yes' if use_07_metric else 'No'))
        if not os.path.isdir(evaluator.output_dir):
            os.mkdir(evaluator.output_dir)
        for i, cls in enumerate(evaluator.labelmap):
            filename = evaluator.get_voc_results_file_template(cls)
            rec, prec, ap = evaluator.voc_eval(detpath=filename,
                                          classname=cls,
                                          cachedir=cachedir,
                                          ovthresh=0.5,
                                          use_07_metric=use_07_metric
            ...
            aps += [ap]
            print('AP for {} = {:.4f}'.format(cls, ap))
            with open(os.path.join(evaluator.output_dir, cls + '_pr.pkl'), 'wb') as f:
                pickle.dump({'rec': rec, 'prec': prec, 'ap': ap}, f)
        if evaluator.display:
            evaluator.map = np.mean(aps)
            print('Mean AP = {:.4f}'.format(np.mean(aps)))
            print('~~~~~~~~')
            print('Results:')
            for ap in aps:
        ...
        if any(lines) == 1:
            ...
            for d in range(nd):
                ...
                if BBGT.size > 0:
                    ...
                    inters = iw * ih
                    uni = ((bb[3] - bb[0]) * (bb[4] - bb[1]) * (bb[5] - bb[2]) +
                           (BBGT[:, 3] - BBGT[:, 0]) *(BBGT[:, 4] - BBGT[:, 1]) * (BBGT[:, 5] - BBGT[:, 2]) -
                            inters)
                    overlaps = inters / uni
